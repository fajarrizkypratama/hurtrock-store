...
def sitemap_xml():
    ...
    try:
        ...
        current_date = datetime.now(ZoneInfo("Asia/Jakarta")).strftime('%Y-%m-%d')
...
def product_detail(slug):
    ...
    valid_until = (
        datetime.now(ZoneInfo("Asia/Jakarta")) + timedelta(days=30)
    ).strftime('%Y-%m-%d')
...
def upload_chat_media():
    ...
    try:
        ...
        timestamp = datetime.now(ZoneInfo("Asia/Jakarta")).strftime('%Y%m%d_%H%M%S_%f')[:19]
...
def _create_midtrans_checkout(cart_items, shipping_service, shipping_cost,
    ...
    order_id = f"ORDER-{current_user.id}-{int(datetime.now(ZoneInfo('Asia/Jakarta')).timestamp())}-{str(uuid.uuid4())[:8]}"
...
def payment_success():
    ...
    if cart_items:
        ...
        order = models.Order(
            ...
            status='paid',
            created_at=datetime.now(jakarta_tz)  # ← langsung waktu Jakarta
        )
    ...
    current_datetime=datetime.now(ZoneInfo("Asia/Jakarta"))
    )
...
def admin_dashboard():
    ...
    # Gunakan timezone Jakarta untuk keakuratan waktu
    ...
    # Total penjualan hari ini (paid/shipped/delivered)
...
def admin_toggle_payment_config(config_id):
    ...
    config.updated_at = datetime.now(ZoneInfo("Asia/Jakarta"))
    ...
    try:
        ...
        if order_id:
            ...
            if midtrans_transaction:
                ...
                midtrans_transaction.updated_at = datetime.now(ZoneInfo("Asia/Jakarta"))
...
def close_cashier_session():
    ...
    if current_user.role not in ['admin', 'staff']:
        return jsonify({
            'success': False,
            'error': 'Akses ditolak. Hanya admin dan staff yang dapat menggunakan kasir.'
        }), 403
    ...
    try:
        ...
        session.session_end = jakarta_time
...
def save_cashier_transaction():
    """Save cashier POS transaction as offline order + offline transaction logs"""
    if current_user.role not in ['admin', 'staff']:
    ...
    try:
        ...
        for item_data in data['items']:
            ...
            if not product:
                return jsonify({
                    'success': False,
                    'error': f'Produk dengan ID {item_data["product_id"]} tidak ditemukan'
                }), 400
            ...
            if product.stock_quantity < quantity:
                return jsonify({
                    'success': False,
                    'error': f'Stok tidak mencukupi untuk {product.name}. Stok tersedia: {product.stock_quantity}'
                }), 400
        ...
        # ---------------------------------------------------------------------
        # 1️⃣ SIMPAN KE OfflineTransaction
        # ---------------------------------------------------------------------
        offline_tx = models.OfflineTransaction(
            ...
            transaction_date=get_utc_time(),
            offline_data=json.dumps(data)
        )

        db.session.add(offline_tx)
        db.session.flush()  # untuk dapat offline_tx.id

        # ---------------------------------------------------------------------
        # 2️⃣ SIMPAN OfflineTransactionItem (item per item)
        # ---------------------------------------------------------------------
        for item in order_items_data:
            ot_item = models.OfflineTransactionItem(
            ...
            db.session.add(ot_item)

        # ---------------------------------------------------------------------
        # 3️⃣ SIMPAN KE Order DAN OrderItem
        # ---------------------------------------------------------------------
        order = models.Order(
            user_id=current_user.id,
            total_amount=server_total,
...
def check_connectivity():
    ...
    return jsonify({
        'status': 'online',
        'timestamp': now_utc.isoformat(),  # ISO format with timezone
        'server_time': now_utc.strftime('%Y-%m-%d %H:%M:%S %Z')  # readable UTC time
    })

def wrap_text(text, max_len=12):
    """Memotong teks menjadi beberapa baris sesuai panjang karakter."""
    if not text:
        return []
    lines = []
...
def print_receipt():
    ...
    total_db = float(order.total_amount)
    total_param = request.args.get('total')
    tunai_param = request.args.get('tunai')

    total = float(total_param) if total_param else total_db
    tunai = float(tunai_param) if tunai_param else total
    ...
    base_height = (70 + (len(items) * 10) + 60) * unit_mm
    ...
    # JUDUL TOKO SUPER BOLD
    p.setFont("Helvetica-Bold", 12)
    nama_toko = store_profile.store_name if store_profile else "HURTROCK MUSIC STORE"
    p.drawCentredString(width / 2, y, nama_toko)
    y -= 7 * unit_mm

    # ALAMAT NORMAL
    p.setFont("Helvetica", 8)
    if store_profile and store_profile.formatted_address:
        for line in wrap_text(store_profile.formatted_address, 50):
    ...
    # ===================== INFO TRANSAKSI =====================
    kasir_name = current_user.name or "Kasir"
    now = datetime.now()

    p.drawString(5 * unit_mm, y, f"Tanggal: {now.strftime('%d/%m/%Y %H:%M')}")
    y -= 4 * unit_mm

    p.drawString(5 * unit_mm, y, f"Kasir: {kasir_name}")
    y -= 4 * unit_mm
    ...
    for item in items:
        ...
        for line in wrap_text(name, 27):
            p.drawString(5 * unit_mm, y, line)
            y -= 4 * unit_mm

        # Harga & subtotal normal
        p.setFont("Helvetica", 8)
        p.drawString(
            7 * unit_mm,
            y,
            f"{item.quantity} x Rp {item.price:,.0f}".replace(",", ".")
        )
    ...
    p.setFont("Helvetica-Bold", 12)  # TOTAL LEBIH TEBAL
...
def admin_create_invoice(order_id):
    ...
    try:
        ...
        invoice_number = f"INV-{jakarta_time.strftime('%Y%m%d')}-{order.id:06d}"
        ...
        invoice = models.Invoice(
            ...
            # Gunakan buyer_name untuk POS (offline), atau nama user untuk online
            customer_name=order.buyer_name or (
                order.user.name if hasattr(order, 'user') and order.user else 'Walk-in Customer'
            ),
            ...
            # Simpan waktu invoice diterbitkan
...
def admin_create_manual_invoice():
    ...
    try:
        ...
        # --- Generate invoice number (WIB) ---
        invoice_number = f"INV-{datetime.now(JAKARTA_TZ).strftime('%Y%m%d')}-{models.Invoice.query.count() + 1:06d}"
        ...
        invoice = models.Invoice(
            ...
            notes=notes,
            issued_by=current_user.id,
            created_at=datetime.now(JAKARTA_TZ),  # jika model punya kolom ini
            updated_at=datetime.now(JAKARTA_TZ)   # jika model punya kolom ini
        )
        db.session.add(invoice)
...
def admin_analytics():
    ...
    try:
        ...
        today_start = now_jkt.replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        today_online_sales = db.session.query(
            ...
            models.Order.created_at < today_end,
            models.Order.status.in_(['paid', 'shipped', 'delivered']),
            models.Order.source_type == 'online'
        ).scalar() or Decimal('0')
        ...
        today_offline_sales = db.session.query(
            ...
            models.Order.created_at < today_end,
            models.Order.status.in_(['paid', 'shipped', 'delivered']),
            models.Order.source_type == 'offline'
        ).scalar() or Decimal('0')

        # ==========================
        #  PENJUALAN BULAN INI (WIB)
        # ==========================
        month_start = now_jkt.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        ...
        best_selling_products = db.session.query(
         ...
         .join(models.OrderItem, models.OrderItem.product_id == models.Product.id) \
         .join(models.Order, models.Order.id == models.OrderItem.order_id) \
         .filter(models.Order.status.in_(['paid', 'shipped', 'delivered'])) \
         .group_by(models.Product.id, models.Product.name) \
         .order_by(func.sum(models.OrderItem.quantity).desc()) \
         .limit(10).all()

        # ==========================
        #  PENJUALAN 7 HARI (WIB)
        # ==========================
        ...
        top_customers_raw = db.session.query(
         ...
         .filter(
             ...
             func.coalesce(func.sum(models.Order.total_amount), 0).desc()
         ).limit(5).all()
        ...
        return render_template(
            ...
            category_sales=[],
            current_date=now_jkt.date(),
            datetime=datetime
        )

    except Exception as e:
        db.session.rollback()
        print("[ERROR] Analytics error:", str(e))
...
def export_sales(period):
    ...
    today = datetime.now(ZoneInfo("Asia/Jakarta")).date()  # gunakan WIB, bukan UTC
    ...
    footer_cell.value = f"Dicetak pada: {datetime.now(ZoneInfo('Asia/Jakarta')).strftime('%d %B %Y %H:%M:%S')} WIB"
    ...
    filename = f'laporan_penjualan_{period}_{datetime.now(ZoneInfo("Asia/Jakarta")).strftime("%Y%m%d")}.xlsx'
...
def admin_update_restock_status(order_id):
    ...
    try:
        ...
        if new_status == 'received':
            restock_order.received_date = datetime.now(ZoneInfo("Asia/Jakarta"))
    ...
    with app.app_context():
        try:
            ...
            store_profile = models.StoreProfile(
                ...
                store_tagline='Toko Alat Musik Terpercaya',
                store_address=
                'Jl Gegerkalong Girang Complex Darut Tauhid Kav 22 Kota Bandung, Jawa Barat 40153 Indonesia',
                store_city='Jawa Barat',
                store_postal_code='40153',
                store_phone='0821-1555-8035',
                store_email='info@hurtrock-store.com',
                store_website='https://hurtrock-store.com',
                whatsapp_number='6282115558035',
                operating_hours=
                'Senin - Sabtu: 09:00 - 21:00\nMinggu: 10:00 - 18:00',
